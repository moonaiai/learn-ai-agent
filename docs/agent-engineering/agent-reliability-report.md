...
| 关键数字 | 含义 | 出处 |
|---|---|---|
| 57.3% / 40% | 已部署生产 Agent / Gartner 预测 2027 年前被取消的项目比例 | [3][6] |
| 32% / 24.9% / 20% | 质量 / 安全 / 延迟作为头号障碍的组织占比 | [3] |
| 41-86.7% | 无容错设计的 Multi-Agent 系统失败率 | [11] |
| 17× | "Bag of Agents" 反模式的错误放大倍数 | [10] |
| 89% / 71.5% | 已实现可观测性 / 具备全链路追踪的生产 Agent 占比 | [3] |
| k=10 时 100% vs ≈0% | pass@k 与 pass^k 的理论极端分化 | [1] |
| 4 维度 | 一致性、鲁棒性、可预测性、安全性 | [4] |
...
> **一句话结论**：Agent 在 Notebook 里完美、在生产中崩溃——失败方式可预测（范围越界、资源失控、不确定性行为），因此也可工程化地防御。
...
![部署 Agent 的组织面临的生产障碍：质量 32%、安全 24.9%、延迟 20%](figures/rel-01-reliability-gap.svg)
...
> **一句话结论**：可靠性与能力是独立维度——"18 个月来准确率持续提升，可靠性仅有微弱改善"，单纯升级模型解决不了可靠性问题。
...
| 维度 | 衡量内容 | 关键发现 |
|---|---|---|
| 一致性（Consistency） | 多次执行的结果可重复性 | 关键指标：结果一致性 C_out（归一化方差）、轨迹一致性（Jensen-Shannon 散度和 Levenshtein 距离）、资源一致性 C_res（变异系数）；"what but not when"模式——Agent 会选择相似的动作类型但执行顺序不同 [4] |
| 鲁棒性（Robustness） | 扰动下的优雅降级能力 | 反直觉发现：Agent"对技术故障的处理反而比对指令改写的处理更好" [4] |
| 可预测性（Predictability） | 置信度与准确率的对齐 | 近期 Claude 系列模型展现出"良好对齐的置信度估计" [4] |
| 安全性（Safety） | 失败时危害的限制 | 被排除在聚合分数之外："重要的不是平均安全性，而是任何严重违规的存在"——它是硬约束 [4] |
...
> **一句话结论**：在概率性输出外包一层确定性验证——输入验证、输出过滤、架构约束三层协同，结构化输出是单一最有效的输出护栏。
...
![多层护栏架构：输入验证与输出过滤包裹 LLM，架构约束作为最外层的确定性笼子](figures/rel-02-guardrail-layers.svg)
...
> **一句话结论**：无容错设计的 Multi-Agent 系统失败率高达 41-86.7%——重试、熔断、降级铁三角加上状态机、检查点与死信队列，构成完整的容错工具箱。

无容错设计的 Multi-Agent 系统失败率高达 41-86.7% [11]。

| 容错模式 | 机制与要点 |
|---|---|
| 重试 | 2-3 次重试 + 指数退避 + 随机抖动 [14]；区分可重试（429/5xx）和需重构（400）的错误；每个外部依赖应有独立的重试策略对象 [5] |
| 熔断器 | 三态切换（Closed → Open → Half-Open）[13]；对 AI 管道尤为重要：下游故障时继续调用 LLM 只会浪费 Token [5] |
| 优雅降级 | 模型回退（GPT-4 → GPT-4o-mini）、缓存回退、功能降级、人工升级 [12] |
| 状态机 | 提供有限的合法状态集、明确的恢复点、防止无效状态转换，优于链式管道——后者可能让 Agent 处于模糊状态，重启后无从恢复 [5] |
| 检查点架构 | 每步持久化状态（步骤 ID、输出数据、状态标志），管道重启时从最后完成步骤恢复 [5] |
| 死信队列 | 失败项路由到队列（含原始输入、检查点、错误详情），支持人工从失败点重新提交 [5] |

![容错铁三角：重试、熔断器、优雅降级相互配合，状态机、检查点与死信队列作为底座](figures/rel-03-fault-triangle.svg)

---

### Finding 5: 可观测性工程——从黑盒到玻璃盒

> **一句话结论**：可观测性是评估的前提——89% 采用率远超评估的 52%，成熟度阶梯是"可观测性 → 离线评估 → 在线评估 → 持续监控"。
...
| 层级 | 覆盖范围 | 代表工具与开销 |
|---|---|---|
| L1 | Prompt / Output | Langfuse、Galileo（约 15% 开销） |
| L2 | Workflow / Eval | LangSmith（约 0%）、Arize、W&B Weave |
| L3 | Agent Lifecycle | AgentOps（约 12%） |
| L4 | Infrastructure | Datadog、Prometheus、Grafana |
...
> **一句话结论**：大多数 Agent"有能力但不可靠"——k=10 时 pass@k 趋近 100% 而 pass^k 跌至 0%，自动化场景必须以 pass^k 为上线门槛。
...
![k=10 时的理论极端：pass@k 趋近 100% vs pass^k 跌至 0%（p=50%）](figures/rel-04-pass-k-decay.svg)
...
> **一句话结论**：错误在 Multi-Agent 系统中以乘法传播——"Bag of Agents" 反模式放大 17 倍，协调拓扑而非 Agent 数量才是解法。
