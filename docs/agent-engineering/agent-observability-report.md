...
| 关键数字 | 含义 | 出处 |
|---|---|---|
| 89% / 62% | 已为 Agent 部署可观测性 / 具备详细追踪能力的团队占比 | [1] |
| 10-100× | Agentic 工作负载相对等效单轮对话的 token 消耗倍数 | [2] |
| 24× | 到 2030 年企业 token 需求的增长倍数（Goldman Sachs 预测） | [3] |
| $5-8 | 未经约束的 Agent 解决单个软件工程问题的成本 | [3] |
| 98% | 已管理 AI 支出的受访者占比（一年前为 63%） | [5] |
| 62% | 重发上下文占 Agent 推理账单的比例 | [3] |
| 8.5× | 无上下文隔离的多 Agent 系统相对单 Agent 的 token 消耗 | [4] |
| 70-85% | 五大优化杠杆联合应用可降低的支出 | [16] |
| 90% | Prompt 缓存可实现的最高成本折扣 | [16][19] |
| 97.6% → 2.4% | GPT-4 质数判断准确率在 3 个月内的变化 | [22] |
...
> **一句话结论**：Agent 的非确定性本质要求可观测性三支柱（Traces / Metrics / Logs）从"基础设施监控"升级为"AI 行为观测"，而非简单复用传统 APM 方法。
...
![传统线性调用链 vs Agent 树状 trace：模型调用与工具执行并列，子 Agent 递归嵌套](figures/01-trace-tree.svg)
...
| 元素 | 名称 | 语义 |
|---|---|---|
| Span | `invoke_agent` | Agent 调用（trace 根节点） |
| Span | `chat` | LLM 推理调用 |
| Span | `execute_tool` | 工具执行 |
| 属性 | `gen_ai.request.model` | 模型标识 |
| 属性 | `gen_ai.usage.input_tokens` / `gen_ai.usage.output_tokens` | token 用量 |
| Metric | `gen_ai.client.operation.duration` | 操作耗时 |

主要的可观测性厂商已开始原生支持这些约定——Datadog 在 OTel v1.37 中开始了原生支持，Grafana 也开始在 Loki 中采集 LLM traces [9]。
...
> **一句话结论**：市场分化为传统 APM、AI-native 追踪与 AI 网关三大阵营；可观测性的"有"已经普及，但"追踪 + 评测 + 告警"的闭环深度仍是大多数团队的短板。
...
| 工具 | 阵营定位 | 许可与部署 | 核心优势 |
|---|---|---|---|
| LangSmith | AI-native 追踪（LangChain 生态） | 专有闭源，自托管仅企业级附加选项 [7] | LangChain/LangGraph 零配置；SmithDB 使 trace 加载仅 92ms；告警路由 PagerDuty / Dynatrace / webhooks |
| Langfuse | AI-native 追踪 | MIT 开源，自托管是一等部署模式 [7] | LLM-as-judge 与 Code Evaluators 开源；Monitors 分级告警；按摄入 units 计费而非席位 |
| Arize Phoenix | AI-native 追踪 + 评测 | 开源（9,000+ GitHub stars）[11] | 基于 OTel / OpenInference；本地单函数启动、无供应商锁定；50+ 评测指标可在 CI 运行 |
| Braintrust | 评测驱动开发平台 | 商业，免费层最慷慨 [12] | 每月 100 万 trace spans、无限用户；"评测优先"连接测试与生产 |
| OpenLLMetry | 标准化检测层 | 开源（7,200+ stars）[12] | 完全基于 OTel 标准，可直接导入既有 APM，可移植性最好 |
| Portkey / Helicone | AI 网关 | Portkey 已于 2026 年 3 月以 Apache 2.0 开源 [13] | 应用与 LLM 提供商间的控制平面：路由、缓存、成本追踪、A/B 测试 |
...
> **一句话结论**：token 是 Agentic AI 的经济原语；重试放大、上下文膨胀与多 Agent 成本放大等隐性成本，可使实际成本达到表面成本的 10 倍。
...
**主流模型定价对比（2026年中数据）。** 当前主流模型的定价已形成清晰的层级结构 [14]：

| 层级 | 模型 | Input / Output（$/1M tokens） |
|---|---|---|
| 旗舰推理 | OpenAI GPT-5.4 | $2.50 / $15.00 |
| 旗舰推理 | OpenAI GPT-5.4 Pro | $30.00 / $180.00 |
| 旗舰推理 | Anthropic Claude Opus 4.7 | $5.00 / $25.00 |
| 旗舰推理 | Google Gemini 3.1 Pro（≤200K context） | $2.00 / $12.00 |
| 旗舰推理 | Google Gemini 3.1 Pro（>200K context） | $4.00 / $18.00 |
| 成本效率 | Anthropic Claude Haiku | 约 $1.00 / $5.00 |
| 成本效率 | Google Gemini 3.1 Flash-Lite | $0.10 / $0.40 |
| 成本效率 | DeepSeek V3 | $0.27 / $1.10 |

**三类 token 的成本结构差异。** 当代 LLM API 的 token 计费分为 input tokens（输入）、output tokens（输出）和 reasoning tokens（推理）三类 [15]：

| 类型 | 计费方式 | 可见性 | 关键陷阱 |
|---|---|---|---|
| Input tokens | 基准单价 | 完全可见 | 多轮对话中历史上下文被反复重发 |
| Output tokens | 通常是 input 的 3-6 倍，反映自回归解码的高计算成本 | 可见 | 生成成本随输出长度线性放大 |
| Reasoning tokens | 按 output token 费率计费 | API 响应中不可见，但消耗上下文窗口 | 500 token 的可见响应可能实耗 2,000+ token；OpenAI o3 单次调用可能在回答前消耗 50,000 output tokens 的 chain-of-thought |

这意味着使用推理模型时，实际成本可能是表面成本的5到10倍。
...
**隐性成本：Multi-Agent 的成本放大效应。** 这是最容易被低估的成本维度，具体量级如下：

- 简单的工具调用 Agent 每任务消耗 5,000-15,000 token，而复杂的多 Agent 系统可消耗 200,000 到超过 1,000,000 token [4]
- 一个三 Agent pipeline 大约消耗 29,000 token 来完成单 Agent 用 10,000 token 就能处理的任务——协调开销在每次交接时都会产生 token 成本 [4]
- 一个五 Agent 系统、50 个推理步骤、8,192 token 的规划文档：仅上下文广播就产生超过 200 万 token 的开销 [4]
- 未经适当上下文隔离的多 Agent 系统，消耗的 token 可达单 Agent 执行相同任务的 8.5 倍 [4]

从微观看，单次 LLM API 调用可能仅花费 $0.001，但一个多步骤 Agentic 决策周期的成本为 $0.10 到 $1.00——这是100到1,000倍的乘数 [4]。Agentic 工作负载整体消耗的 token 量是等效单轮聊天交互的10到100倍 [2]。

多 Agent 系统中最危险的经济陷阱是二次 token 增长——在多轮对话中成本快速累积，没有显式的上下文纪律，多 Agent 系统会随着 Agent 数量增加而指数级增长成本 [2]。

![多 Agent 成本放大阶梯：单 Agent 10,000 token → 三 Agent 29,000 token → 无隔离多 Agent 约 85,000 token](figures/02-multi-agent-cost.svg)

这些隐性成本意味着，一个看似合理的 $6/session 成本，在未经优化的情况下可能实际运行在 $60/session 的水平。理解和管控这些隐性成本是 Agent 系统走向生产的必要条件。

### 发现四：成本优化的四大杠杆

> **一句话结论**：模型路由、语义缓存、Prompt 缓存、上下文压缩与批处理五大杠杆联合应用，可将支出降低 70-85%——但每种优化都有适用条件和权衡。

面对 Agent 系统的高昂 token 成本，业界已发展出四大系统性优化杠杆。五个优化杠杆（模型路由、语义缓存、Prompt 缓存、上下文压缩、批处理）联合应用可将支出降低70-85%，使 $6/session 降至 $0.90-1.80 [16]。

| 杠杆 | 核心原理 | 典型效果 | 关键权衡与适用条件 |
|---|---|---|---|
| 模型路由 | 评估请求难度，常规任务给小模型、复杂推理给前沿模型；生产流量中 60-80% 的请求足够简单 [17] | 实施后 40-70% 节省；调优后 37-46%，极端配置可接近 79% [17] | 需要质量反馈闭环校准；校准正确时不会降低输出质量 [17] |
| 语义缓存 | 嵌入 + 向量相似度命中语义相近的历史查询 [18] | GPTCache 命中率 61.6-68.8%，正面命中准确率超 97% [18] | 命中率与新鲜度的张力；相似度阈值需仔细设定 |
| Prompt 缓存 | 缓存 prompt 静态前缀：静态内容在前、动态内容在后 | 缓存读取 90% 折扣、延迟降低 85%（写入为正常输入的 1.25×）[16][19] | 收益高度依赖 prompt 结构工程（ProjectDiscovery 命中率 7%→84%，削减总支出 59-70% [19]） |
| 上下文压缩 | 摘要化、关键短语提取、语义分块 | 5-20× 压缩，50-70% token 减少 [16][21] | 提取式通常优于抽象式，避免摘要过程引入幻觉 [21] |
| 批处理 | Batch API 异步处理非实时任务（5万-10万请求/批） | 50% 折扣，输出不变 [20] | 仅适用离线评测、文档处理等非实时场景；与 Prompt 缓存叠加综合节省可超 95% [20] |
...
![五大优化杠杆联合应用：$6.00/session 降至 $0.90-1.80，总支出降低 70-85%](figures/03-optimization-levers.svg)

### 发现五：模型漂移与质量监控

> **一句话结论**：模型漂移真实且剧烈——GPT-4 可在 3 个月内从接近完美变为几乎完全失效；没有持续质量监控的生产系统，等于在赌模型提供商的下一次更新。
...
![GPT-4 与 GPT-3.5 在质数判断任务上的准确率变化：3 个月内 GPT-4 从 97.6% 跌至 2.4%，GPT-3.5 从 7.4% 升至 86.8%](figures/04-drift-case.svg)
...
- **Population Stability Index（PSI）**：超过 0.2 通常表示显著漂移
- **Kolmogorov-Smirnov 检验**：可捕捉连续分布的形状变化，但由于 embedding 的多维性质，效果有限
- **Wasserstein 距离**：及类似度量在多维空间中表现更好
- 其他方法：欧几里得距离和余弦距离、Maximum Mean Discrepancy（MMD）、基于模型的漂移检测、追踪漂移 embedding 的比例等
...
1. 建立性能基线数据集，定期（至少每周）运行回归测试；
2. 在生产流量上部署采样评估，覆盖核心质量维度（忠实度、相关性、幻觉率）；
3. 使用统计过程控制（SPC）方法设定动态阈值，而非固定阈值；
4. 将漂移检测结果与模型版本、provider 更新日志关联，建立因果分析能力；
5. 制定漂移响应预案——包括回滚到上一版本、切换到替代模型、或降级到更保守的 prompt 策略。

### 发现六：生产监控仪表盘与告警设计

> **一句话结论**：生产仪表盘需要延迟、资源、质量、业务四层指标；SLO 与"固定阈值 + 异常检测"的分层告警，让异常在成为危机之前浮现。

**关键指标体系。** 生产 LLM 监控仪表盘的指标体系可分为四个层次：

| 层次 | 核心指标 | 回答的问题 | 要点 |
|---|---|---|---|
| 延迟 | TTFT、端到端延迟、Inter-token latency、P50/P95/P99 分位数 [25] | 用户是否在等待 | 平均值正常不代表尾部正常：100 RPS 下平均 TTFT 120ms 看似正常，但 P99 达 600ms 意味着每秒有一个用户在看到任何输出前等待半秒。P99 可捕捉 GC 暂停、KV cache 淘汰、请求排队、冷启动等系统性问题的尾部尖峰 [25] |
| 资源消耗 | 每请求 token 数（区分 input/output/reasoning）、每请求成本、每用户/每功能的成本归因 | 钱花在了哪里 | 成本工程的基础数据 |
| 质量 | 业务语义成功率、幻觉率（LLM-as-a-judge 或规则检测）、groundedness、citation coverage、工具调用成功率、用户反馈分数 [24] | 输出是否有意义 | 传统 APM 最缺乏的层面，也是 AI-native 可观测性的核心差异点 |
| 业务 | 每会话成本、每任务完成成本、ROI（节省的人工成本 vs LLM 支出） | 技术投入是否产生业务价值 | 将技术运营与业务价值关联 |

**SLO 设计。** TTFT（Time to First Token）的用户体验阈值已有清晰的经验数据 [25]：

| TTFT P99 | 用户感知 |
|---|---|
| 300ms | 用户停止注意到延迟的阈值——文本开始流出前的等待感不被察觉 |
| 500ms | 大多数用户感知到延迟 |
| 800ms | 会话放弃率可测量地增加 |

基于这些经验数据，SLO 设计建议如下：对于交互式 Agent 应用，TTFT P99 SLO 应设定在300-500ms 范围；对于 batch 处理场景，可放宽至秒级。

Token 消耗的 SLO 同样重要但常被忽略——为每种 Agent 任务类型设定 token 预算上限，超出则触发告警。这不仅控制成本，还能及时发现 Agent 行为异常（如陷入循环推理或无限工具调用）。

**告警策略：异常检测 vs 固定阈值。** 固定阈值告警简单直观，但容易产生告警疲劳或遗漏渐变型退化。生产推荐的做法是使用分层告警策略：

| 层级 | 机制 | 典型配置 |
|---|---|---|
| P1（立即响应） | 固定阈值 | 错误率超过 5%、P99 延迟超过 SLO 的 2 倍 |
| P2（调查） | 异常检测：基于历史数据的滑动窗口计算动态基线，偏差超过标准差倍数时触发 | 质量指标（如幻觉率）——"正常"水平会随 prompt 更新、模型版本变化自然波动 |

例如，可以设定规则："如果10分钟内超过5%的聊天响应被标记为'不忠实'（unfaithful），则向值班工程团队发送告警" [24]。异常检测应用于评测分数可在漂移成为危机之前将其浮现——例如，如果幻觉评估分数在一小时内超过5%的 traces 低于3分，就需要调查 [24]。
...
**仪表盘设计模式。** 一个生产级 LLM 监控仪表盘应包含以下五个视图：

1. **总览面板**——实时的请求量、成功率、P50/P95/P99 延迟、总 token 消耗和成本的时间序列；
2. **成本分析面板**——按模型、按 Agent 类型、按用户/租户的成本分解，包含趋势线和预算对比；
3. **质量面板**——幻觉率、评测分数分布、用户反馈趋势；
4. **追踪详情面板**——可深入到单个 trace 的树状视图，检查每个 span 的 prompt、响应、token 用量和耗时；
5. **告警面板**——活跃告警列表、历史告警趋势、告警响应时间指标。
...
> **一句话结论**：可观测性的终点不是"记录发生了什么"，而是"理解和改善 AI 的行为"——评测驱动的持续优化闭环与 FinOps for AI 治理。
...
这代表了从"可观测性1.0"（记录发生了什么）到"可观测性2.0"（理解和改善 AI 的行为）的范式转变。AI-native 可观测性的特征包括 [10]：

- 目的化构建的 LLM 调用、Agent 运行、工具调用和检索步骤的 trace 捕获，每个 span 附带 token 计数、成本、延迟和模型参数；
- 内置的评测能力（而非需要外部工具）；
- 语义级别的漂移检测（而非仅基于数值指标）；
- 从观测到优化的闭环能力。
...
1. **生产可观测性**——持续收集 traces、metrics 和评测数据；
2. **自动化评测管道**——将观测数据转化为质量信号和优化建议；
3. **持续部署（CD）集成**——将评测结果作为部署门控，确保只有通过质量基线的变更才能进入生产。

![评测驱动的持续优化闭环：生产可观测性 → 自动化评测管道 → 优化迭代 → CD 质量门控](figures/05-observability-loop.svg)
