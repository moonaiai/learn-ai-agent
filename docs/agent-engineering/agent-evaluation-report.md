...
| 关键数字 | 含义 | 出处 |
|---|---|---|
| 57.3% / 52.4% / 37.3% | 已部署生产 Agent / 运行离线评测 / 运行在线评测的组织占比 | [1] |
| 53.3% | 使用 LLM-as-Judge 进行评测的团队占比 | [5] |
| 500-5,000× | LLM-as-Judge 相对人类评审的成本降低（约 80% 人类偏好一致性） | [5] |
| 41.3% | 交换回答顺序后 LLM 评委翻转判断的中位比例 | [6] |
| 15-30 分 | 冗长偏差造成的偏好膨胀幅度 | [7] |
| 61% → 25% | GPT-4o 在 tau-retail 上 pass^1 → pass^8 | [8] |
| 20-25 个百分点 | SWE-bench Pro 相对 Verified 拉低的同模型得分 | [3] |
| 37% | 实验室分数与生产表现之间的性能差距 | [18] |
| 28 分钟 / 40 小时 | 自主 AI 渗透测试匹配资深测试员 85% 得分所需时间对比 | [14] |

![部署-评测鸿沟：57.3% 组织已部署生产 Agent，但仅 52.4% 运行离线评测、37.3% 运行在线评测](figures/eval-01-deploy-eval-gap.svg)
...
> **一句话结论**：静态基准正因数据污染而遭遇信任危机——同一模型在 Verified 与 Pro 上得分相差 20-25 个百分点，评测范式正在从静态转向动态。
...
| 基准 | 定位与规模 | 关键发现与数据 |
|---|---|---|
| SWE-bench Verified | 代码 Agent 修复真实 GitHub issue，500 道题 [2] | 2024年6月后基于 GitHub 数据训练的模型可能见过题目及解答 [2]；LLM 仅靠记忆可达 76% 准确率 [2]；OpenAI 审计发现 138 道 GPT-o3 未稳定解决的题目中 59.4% 存在实质缺陷，过滤后有效解决率从 22.4% 骤降至 10.0%，已于 2026年2月弃用 [3] |
| SWE-bench Pro | Scale AI 推出：1,865 个任务、41 个代码仓库，含私有商业分片 [3] | 将同一模型得分拉低 20-25 个百分点：GPT-5 与 Claude Opus 4.1 仅得 23.3% 和 23.1%（Verified 上超 70%）[3]；私有子集进一步拉开：Claude Opus 4.1 降至 17.8%，GPT-5 降至 14.9% [3] |
| GAIA | 多步推理试金石，466 道题（Meta AI / Hugging Face / AutoGPT，ICLR 2024）[15] | 人类正确率 92% vs 配插件 GPT-4 仅 15% [15]；三档难度（<5 步 / 5-10 步多工具 / 复杂推理链）[15]；2026 年 CustomGPT.ai 达 93.36%，引发基准饱和担忧 [15] |
| WebArena / WebChoreArena | 真实 Web 环境交互评测，812 任务覆盖 Magento、Postmill、GitLab 等 [16] | 两年内 Agent 成功率从 14% 升至约 60% [16]；WebChoreArena 引入 532 个高难度任务——Gemini 2.5 Pro 在 WebArena 达 54.8%+ 但在 WebChoreArena 仅 37.8% [16] |
| tau-bench | Sierra AI 开发，模拟客服 Agent 动态对话（零售/航空领域 + API 工具 + 策略指南）[8] | 核心创新：引入 pass^k 指标衡量 k 次独立试验的一致性（详见发现三） |
| HumanEval / MBPP | 功能性代码生成：164 + 974 道 Python 题 [17] | 前沿模型已超 90 分、基准饱和 [17]；编程概念覆盖不到 53%、超 80% 问题"容易"、57% 输出含训练数据记忆化片段 [17] |
| SWE-MERA | 动态基准代表：七阶段流水线筛出约 300 个污染最小化任务，月度更新 [4] | 研究发现 SWE-bench 有 32.67% 成功补丁涉及直接解决方案泄露、31.08% 因测试用例不足而通过 [4] |

**从静态到动态的范式转换。** 静态基准的根本问题在于：固定数据集可以被记忆或泄露，一旦受到污染就无法恢复，且企业实际数据显示实验室分数与生产表现之间存在37%的性能差距 [18]。SWE-MERA通过七阶段流水线从数百万GitHub issue和PR中筛选出约300个污染最小化的任务核心，并实施月度更新 [4]。SWE-MERA的月度发布周期和时间滑块排行榜界面为未来动态基准树立了范式 [4]。

![同一模型在 SWE-bench Verified 与 SWE-bench Pro（含私有子集）上的得分落差](figures/eval-02-benchmark-drop.svg)
...
> **一句话结论**：LLM-as-Judge 以 500-5,000× 的成本优势成为主流，但位置、冗长、自我偏好三大系统性偏差意味着"不校准就使用"等于"以可复现的方式悄然出错"。
...
![LLM-as-Judge 位置偏差：平均首选第一位 64.3%，中位顺序翻转率 41.3%，个别模型高达 72.5%](figures/eval-05-judge-bias.svg)
...
1. **Gold Set 对齐**——维护一组经人类专家标注的黄金标准评判案例，定期将LLM评委的输出与之对齐，校准评委的判断标准；
2. **多评委集成**——使用多个不同模型家族的LLM评委并聚合其判断，既缓解单一模型的特有偏差，也利用跨模型家族评审来对抗自我偏好 [19]；
3. **顺序随机化**——在成对比较中随机化候选回答的展示顺序，或执行双向评判后取保守聚合 [6]；
4. **事后定量校准**——对LLM赋予的分数进行事后定量校准或不确定性估计，以调整分数、识别不可靠的判断并与人类评分对齐 [7]。
...
> **一句话结论**：pass@k 回答"能不能做到"，pass^k 回答"能不能可靠地做到"——生产环境必须以后者作为核心指标，因为它随 k 指数衰减。
...
![pass@k 与 pass^k 的剪刀差（p=50%）：多试几次趋近必然成功 vs 每次都成功的概率指数衰减至零](figures/eval-03-pass-k-curves.svg)
...
> **一句话结论**：89% 有可观测性但仅 52.4% 做离线评测——能看到 Agent 在做什么不等于知道它做得好不好；三层框架 + 共享数据层是收敛中的最佳实践。
...
![端到端评测流水线：离线评测与在线评测共享统一数据层，CI/CD 门禁串起部署前质量关卡](figures/eval-04-pipeline.svg)
...
> **一句话结论**：非确定性使精确匹配测试失效——Shadow Mode 零风险验证、Golden Set 锚定回归、门禁区分"能力"与"回归"，构成分层的质量保障体系。
...
Shadow mode的技术实现要点包括：

- 确保两个Agent版本接收完全相同的输入（包括上下文和对话历史）；
- 异步执行Shadow版本以避免增加用户感知的延迟；
- 建立自动化的输出比较流水线，使用多维度评分而非精确匹配；
- 设定明确的Shadow期间和"毕业"标准——当Shadow版本在所有关键指标上至少不逊于当前生产版本时，才可以正式切换。
...
**分层质量保障体系。** 综合以上实践，一个成熟的Agent质量保障体系应包含以下层次：

- **开发阶段**：通过评测驱动开发（EDD）确保每次变更都经过评估；
- **提交阶段**：通过CI/CD回归门禁确保不引入退化；
- **预发布阶段**：通过Shadow mode在真实流量上验证；
- **发布阶段**：通过金丝雀部署或A/B测试逐步放量；
- **生产阶段**：通过在线评测和漂移检测持续监控 [23][24]。

每一层的评测方法和阈值应根据Agent类型和业务风险等级进行定制。

---

### 发现六：评测框架与工具生态

> **一句话结论**：评测工具生态呈"双框架"格局——开发评测（DeepEval/Promptfoo）与生产监控（Braintrust/LangSmith）各司其职，选型看设计哲学而非特性清单。

**生态概览。** AI Agent评测工具生态已形成一个多层次的竞争格局，从聚焦RAG评测的专用框架到提供全栈评测能力的综合平台各有所长。对于工程团队而言，理解每个框架的设计哲学和最佳适用场景，比简单的特性对比更为重要。

| 框架 | 定位 | 核心能力 | 最佳场景 |
|---|---|---|---|
| RAGAS | RAG 评测的学术标杆 | 旗舰指标 faithfulness / answer_relevancy / context_precision / context_recall，学术级方法论、大多无需真实标签 [9]；局限：范围限于检索与生成评分，无内置生产监控或协作层 [26] | RAG 指标是全部工作且需要最小化脚手架时 |
| DeepEval | 测试优先的全栈框架（"LLM 的 pytest"） | 测试用例、断言、50+ 指标、G-Eval、DAG、红队测试、托管仪表板 [10]；通过 assert_test() 与 pytest 深度集成，每次 push/PR 运行相同评测 [10]；G-Eval 用自然语言定义评测指标 [10] | 需要 CI 断言、自定义标准、组件级追踪和测试优先体验时；非 RAG 场景（工具调用、策略遵从、多步推理）的唯一选择 [26] |
| Braintrust | 评测优先的综合平台 | 原生 GitHub Action 在每个 PR 上运行评测，分数低于阈值阻止合并 [11]；更广的框架兼容性，按团队而非按席位定价 [11] | AI 评测需要决定哪些代码进入生产、需要团队级协作时 |
| LangSmith Evaluation | 可观测性优先的评测扩展 | 多种评估器：人工评审、启发式检查、LLM-as-Judge、成对比较 [12]；离线评测 = "单元测试"，在线评测实时评分检测漂移 [12]；traces 显示完整执行树、支持按 session 分组评估 [12] | 团队主要基于 LangChain/LangGraph 构建、需要统一可观测性-评测平台时 [11] |
| Promptfoo | 安全评测与红队专家 | 面向开发者的开源 CLI：系统化 prompt 测试、评测、安全扫描 [11]；红队测试——探测漏洞、注入测试、PII 泄露检查、guardrails 边缘案例 [11]；OpenAI 于 2026年3月宣布收购，红队能力将集成进 Frontier Agent 平台 [11] | 安全评测和红队测试是首要关注点时 |
...
> **一句话结论**：EDD 把评测从"部署前的验收"变成"贯穿开发周期的导航系统"——先定义质量标准，再让每次变更成为有明确结果的可测量实验。
...
**EDD的三步核心循环。** EDD的实践流程可以提炼为一个三步循环 [13]：

1. 策划约100条黄金测试用例；
2. 定义3-5个与应用表现相关的指标；
3. 迭代直到这些指标通过。

更精确地说：定义"好的输出"是什么样子，将这些定义编码为评测，使用评测分数作为判断标准；如果评测正确地捕捉了质量，那么提高分数就意味着改善产品，每一次prompt调整、模型切换和管道变更都变成一个有明确结果的可测量实验 [13]。
...
> **一句话结论**：Agent-as-Evaluator、自动化红队（AI 攻击 AI）、动态基准与多维评测正在把评测推向"可靠性科学"。
...
- **SWE-MERA**：通过月度更新和时间滑块排行榜UI来对抗数据泄露 [4]；
- **AntiLeak-Bench**：使用明确在LLM训练集中不存在的新知识来构造样本，确保严格的无污染评测，并设计完全自动化的工作流程来构建和更新基准、无需人工劳动 [29]。

这些动态基准的核心设计原则是：持续引入新数据以超越模型训练截止日期、自动化质量验证流水线以降低维护成本、以及透明的污染检测机制。
