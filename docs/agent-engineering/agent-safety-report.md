...
| 关键数字 | 含义 | 出处 |
|---|---|---|
| 97% / 14.4% | 预期本年度发生重大 AI Agent 安全事件 / 实施了完整安全控制的组织占比 | [1] |
| 34% | 受 Prompt 注入影响的已部署 Agent（OWASP LLM Top 10 榜首） | [1] |
| 72.8% | MCPTox 基准测得的工具投毒攻击成功率 | [7] |
| 84.30% | ASB 基准测得的最高平均攻击成功率 | [22] |
| 86% → 4.4% | Constitutional Classifiers 上线前后的越狱成功率 | [25] |
| 3 → 121 篇 | Multi-Agent 安全研究论文数量（2023 → 2025） | [2] |
| N(N-1)/2 | N 个 Agent 系统的潜在通信通道数——每个都是攻击向量 | [2] |
| 84% | Claude Code 沙箱在内部使用中减少的权限提示 | [18] |
| 3500万欧元 / 7% | EU AI Act 违反禁止性 AI 实践的最高罚款 | [26] |

![安全防御准备度缺口：97% 预期重大安全事件 vs 仅 14.4% 实施完整安全控制](figures/safety-01-readiness-gap.svg)
...
> **一句话结论**：OWASP 2025 版标志着风险重心从"模型本身的弱点"转向"系统级与架构级风险"——Prompt 注入蝉联榜首，间接注入首次正式同级化。
...
| # | 风险 | 核心要点 |
|---|---|---|
| LLM01 | Prompt 注入 | 继续蝉联榜首。2025版首次将间接注入（indirect injection）正式列为与直接注入同等级的子类型 [4]——Agent 通过 RAG、工具调用和外部数据源接触不可信输入的场景急剧增加。缓解：输入验证、权限分离、将 LLM 输出视为不可信数据 |
| LLM02 | 敏感信息泄露 | 排名变化最大：从2023版第6位跃升至第2位 [5]。从 PII 到企业机密和系统架构详情；向外部 LLM 提供商发送 Prompt 意味着 PII 离开组织基础设施边界 [6] |
| LLM03 | 供应链风险 | 从第5位升至第3位 [5]。预训练模型、微调数据集、Prompt 模板库、插件和 MCP 服务器的每个环节都可能成为攻击入口；MCP 工具投毒是典型体现 [7] |
| LLM04 | 数据与模型投毒 | 聚焦训练/微调数据完整性。攻击者可向向量数据库注入恶意内容投毒 RAG 系统，检索到被投毒文档时隐藏指令可引导 AI 执行攻击者意图 [8] |
| LLM05 | 不当输出处理 | LLM 输出不应被直接信任或执行。输出传递给下游系统（数据库查询、API 调用、代码执行）时缺乏验证可导致注入传播——Agent 输出通常直接触发工具调用，风险尤甚 |
| LLM06 | 过度代理 | 2025版显著扩展，分解为三个根本原因：过度功能（超出任务范围的工具）、过度权限（超出必要的特权）、过度自主（无人监督的高影响操作）[3]。已部署系统中最具预测性的故障模式 [1] |
| LLM07 | 系统提示泄露 | 2025版新增。攻击者可提取系统指令、安全规则和内部配置——既泄露知识产权，又为后续攻击提供侦察信息 |
| LLM08 | 向量与嵌入弱点 | 2025版新增，回应 RAG 主流化 [3]。向量数据库访问控制不足可致跨租户暴露；攻击者还可操纵嵌入模型产生误导性相似度结果 [3] |
| LLM09 | 错误信息 | 由2023版"过度依赖"重命名并重新聚焦。不仅在于用户过度信任输出，更在于模型本身生成和传播虚假信息 [3]；在 Agent 中可触发错误的决策链和工具调用 |
| LLM10 | 无界消耗 | 替换2023版较窄的"模型拒绝服务"，涵盖完整资源滥用频谱：DoS、财务利用（高额 API 账单）、模型未授权复制 [3]。当使用成本足够大，成本本身成为攻击向量 |
...
> **一句话结论**：Prompt 注入是 LLM 时代的"SQL 注入"——间接注入、多步杀伤链与跨 Agent 传播让攻击面从输入层扩展到协议层。
...
| 案例 | 手段与后果 |
|---|---|
| Bing Chat 间接注入（2023） | 斯坦福学生通过精心构造的 Prompt 使 Bing Chat 泄露了其隐藏的系统指令和内部配置 [11]；后续研究展示通过网页隐藏文本实现更复杂攻击，包括数据外泄 |
| ChatGPT 记忆注入 | 恶意网站在 ChatGPT 网页浏览功能中注入 Prompt——页面中的隐藏指令可指导 ChatGPT 执行用户从未请求的操作 [11]。2024年底 The Guardian 测试发现该功能仍然易受间接注入攻击 [11] |
| MCP 工具投毒（2025） | Invariant Labs 命名的攻击：在计算器工具描述中隐藏指令，使 Cursor 编辑器读取用户私有 SSH 密钥并发送出去 [7]。此后已泄露 WhatsApp 聊天记录、GitHub 私有仓库和 SSH 凭证。MCPTox 基准在45个真实 MCP 服务器和20个领先 AI 模型上测得攻击成功率高达 72.8% [7]；两个重大 CVE（MCPoison CVE-2025-54136 和 CurXecute CVE-2025-54135）登上安全版图 [7] |

![MCP 工具投毒攻击链：工具描述隐藏指令 → Agent 无清洗转交模型 → 执行恶意操作 → 数据外发](figures/safety-03-tool-poisoning.svg)
...
> **一句话结论**：护栏的本质是分层防御——永远不要让 LLM 直接调用工具，输入/输出护栏、沙箱与 HITL 组合覆盖不同攻击面。
...
![分层防御架构：输入护栏 → LLM → 输出护栏 → 沙箱内的工具执行与 HITL 审批，全程监控审计](figures/safety-02-defense-in-depth.svg)

**输入护栏（Input Guardrails）** 在用户请求到达 LLM 之前执行过滤和检测，关键能力包括：

- **意图分类**——第一道防线，通过分类器判断用户请求是否属于 Agent 的合法业务范围，超出范围的请求在到达 LLM 之前即被拦截，显著减少攻击面；
- **Prompt 注入检测**——多层技术：基于启发式规则的模式匹配（检测已知攻击模式）、基于 LLM 的分析（专用模型判断注入企图）、基于向量数据库的历史攻击匹配 [14]；
- **敏感词过滤与内容分级**——阻止包含禁止主题、有害内容或超出安全策略范围的输入。

**输出护栏（Output Guardrails）** 在 LLM 响应返回给用户或触发工具调用之前执行验证，关键能力包括：

- **格式验证**——确保输出符合预期结构化格式（JSON schema、API 参数规范等），防止格式错误触发下游系统故障；
...
| 工具 | 机制与优势 | 适用场景 |
|---|---|---|
| NeMo Guardrails（NVIDIA） | 通过对话流控制实现安全防护：Colang 2.0 领域特定语言定义跨五个管道阶段（输入/对话/检索/执行/输出轨道）触发的规则 [15]；核心优势是有状态对话管理，能理解上下文并执行复杂对话策略与主题边界 | Agent 需要明确主题边界和复杂对话策略时 |
| Guardrails AI | 开源程序化框架，通过输出验证缓解 LLM 使用风险：Python/JavaScript + Guardrails Hub 预构建验证器 [15]；核心优势是结构化数据验证 | LLM 必须返回表单、API 载荷或报告等结构化数据时 [15] |
| LLM Guard | 快速第一层扫描器：输入/输出扫描系列（注入检测、越狱字符串、密钥泄露、Token 限制、禁止主题、匿名化），在管道前端拦截低成本明显攻击 [15] | 面向真实用户的生产应用，需要开箱即用的广泛覆盖 |
| Rebuff（Protect AI） | 开源自强化注入检测：四层防御（启发式过滤、基于 LLM 的检测、向量库历史攻击匹配、金丝雀令牌泄露检测）[14]。注意：仍是原型阶段，不能提供100%的注入防护 [14] | 注入检测的辅助层 |
...
> **一句话结论**：与其监督 Agent 做什么，不如监督它能做什么——微虚拟机隔离、最小权限与 HITL 分层审批是沙箱化的三件套。
...
| 技术 / 平台 | 机制 | 关键数据与采用方 |
|---|---|---|
| Firecracker 微虚拟机 | 当前执行不可信代码的黄金标准：每个工作负载独立内核，运行在硬件虚拟化（KVM）之上，一个 VM 内的内核漏洞无法触及宿主机或其他 VM [16] | 启动约 125ms、内存开销约 5MB [16]；驱动 AWS Lambda、E2B、Vercel Sandbox |
| gVisor 用户空间内核 | Google 开发的用户空间应用，拦截并重新实现系统调用（syscall），沙箱程序永不与真实内核通信 [16] | 安全性强于容器、开销低于完整 VM [16]；Google（GKE Agent Sandbox）、Modal 采用 |
| 加固容器 | 最轻量方案，但共享内核的本质使其不足以隔离 LLM 生成的不可信代码 [16] | 对 LLM 生成代码执行，微虚拟机（Firecracker、Kata）是唯一的生产安全隔离层 [16] |
| E2B | 专为 AI Agent 工作流构建的安全沙箱，SDK 优先设计，基于 Firecracker 提供临时代码执行环境 [16] | 强项：专为 AI Agent 场景优化的 API 和短暂性执行模型 |
| Modal | 基于 gVisor 隔离，为 Python ML 工作负载优化 [16] | 原生 GPU 支持（T4 到 H200）、无服务器自动扩展、Python SDK 基础设施即代码 |
...
> **一句话结论**：泄露路径多样且隐蔽，防线要横跨 PII 脱敏、多租户隔离、数据分级与 RAG 权限传播——任何单层的失败不应导致完全妥协。
...
| 隔离策略 | 形态 |
|---|---|
| 硬隔离 | 每个租户获得完全独立的部署，包含专用计算和严格分区的数据存储 |
| 软隔离 | 多个租户在相同底层基础设施上运行，使用逻辑边界分离数据 |
| 带访问控制的共享隔离 | 使用行级安全在查询时执行权限检查 |

关键原则是后端服务（而非语言模型）应解析租户上下文，以防止 LLM 上下文窗口成为跨租户数据外泄路径 [19]。租户感知上下文传播意味着 tenant_id、作用域和策略附加到每个工具调用、检索查询和模型请求 [19]。

**数据分级体系：** 生产 AI Agent 系统应建立明确的数据分级策略，每个级别对应不同的安全控制措施：

| 级别 | 安全控制 |
|---|---|
| 公开（Public） | 可自由用于 LLM 处理 |
| 内部（Internal） | 需要身份验证，但可使用外部 LLM API |
| 机密（Confidential） | 仅通过私有部署的模型处理，Prompt 和响应不得离开组织网络边界 |
| 绝密（Top Secret） | 额外的审计跟踪、加密和人工审批流程 |
...
保障 RAG 安全需要在三个不同层——摄入层、检索层和生成层——进行控制，任何单一层的失败不应导致完全妥协 [8]：

- **摄入层**：维护文档级元数据（包括原始权限信息）并将其存储为向量的附属信息；
- **检索层**：使用权限感知的元数据过滤器，确保每次向量查询都受到访问控制约束；
- **生成层**：验证返回给用户的内容不包含其无权访问的信息。

向量数据库隔离的推荐方案是每个租户一个命名空间（namespace），因为查询成本通常基于命名空间大小，使每个租户一个命名空间比使用元数据过滤器扫描单个大命名空间显著更便宜 [19]。

### 发现六：Multi-Agent 安全 — 信任边界与攻击面

> **一句话结论**：Multi-Agent 的安全性是非组合的——单独安全的 Agent 可以组合成不安全的系统，攻击面随 Agent 数量二次方增长。
...
![Multi-Agent 安全研究论文数量：2023 年 3 篇 → 2024 年 42 篇 → 2025 年 121 篇](figures/safety-04-research-growth.svg)
...
> **一句话结论**：红队测试必须从一次性评估走向持续实践——Promptfoo 做 CI/CD 门控、PyRIT 定期全面攻击、Garak 做基线扫描。
...
| 工具 | 机制特点 | 适用场景 |
|---|---|---|
| Promptfoo | 从应用开发者视角测试完整 LLM 系统（RAG 管道、Agent 架构、API 集成），动态生成数千个针对特定应用上下文的攻击变体 [12]。OpenAI 于2026年3月以约8600万美元收购，但保持 MIT 许可证 [12] | CI/CD 集成应用安全测试的最佳默认选择，快速反馈循环 |
| PyRIT（Microsoft） | 工具套件而非单一工具：编排器 LLM 充当攻击者，基于目标模型实时响应动态生成和改进对抗性 Prompt（区别于 Garak 的静态探测库）[12]；标志性能力是多轮对话攻击模拟——渐进式攻击在多次交互中逐步升级 [12] | 定期的全面红队测试 |
| Garak（NVIDIA） | 基于学术研究和已记录漏洞的预定义攻击探测库，安全研究人员可针对 LLM 端点运行以检查已知弱点 [12] | 重大模型变更前的基线漏洞扫描 |
...
![持续红队测试闭环：每次提交/模型更新自动触发安全测试，阈值突破即告警并阻断部署](figures/safety-05-redteam-loop.svg)

### 发现八：前沿趋势与合规治理

> **一句话结论**：合规从可选转向强制——EU AI Act 罚款最高达全球营业额 7%，EU、US、中国三大辖区的监管图景正在成形。
...
*EU AI Act（欧盟人工智能法案）*：这是全球最具约束力的 AI 监管法律 [26]。

关键时间线：

| 时间节点 | 生效内容 |
|---|---|
| 2025年2月 | 禁止类 AI 实践生效（社会评分、非目标面部识别抓取、工作场所和学校情感识别） |
| 2025年8月 | 通用人工智能（GPAI）模型义务生效 |
| 2026年8月 | 广泛执法开始 |
...
| 违规类型 | 罚款上限 |
|---|---|
| 违反禁止性 AI 实践 | 3500万欧元或全球年营业额的 7% |
| 不遵守高风险 AI 系统要求 | 1500万欧元或年营业额的 3% |
| 向当局提供不正确信息 | 750万欧元或年营业额的 1% |

GPAI 提供商必须维护技术文档、发布训练数据摘要、实施合理的风险应对政策，并在模型存在系统性风险时通知 EU AI Office [26]。
