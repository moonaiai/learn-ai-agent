...
| 关键数字 | 含义 | 出处 |
|---|---|---|
| 40% vs <5% | 2026 年底嵌入 AI Agent 的企业应用比例 vs 2025 年（Gartner 预测） | [1] |
| 95% | 无法进入生产的 AI 项目比例（MIT，主因是架构与治理缺失） | [16] |
| 15× | Multi-Agent 系统的 token 开销倍数 | [9] |
| 42% → 78% | 相同模型与 prompt 下，仅修改 Harness 后的基准成功率 | [3] |
| 90.2% | 多 Agent 架构相对单 Agent 的性能提升（Anthropic） | [12] |
| 89% / 52.4% | 已部署可观测性 / 实施离线评测的组织占比 | [5] |
| 57.3% / 33% | 已有生产 Agent / 仍受质量问题困扰的组织占比 | [5] |
| $9 → $200 | Prompt-and-run vs 结构化迭代的生产成本对比 | [3] |
| 1,300+ / 周 | Stripe Minions 系统自动合并的 Pull Request 数 | [3] |
...
> **本章核心**：工程严谨性不会消失，只会迁移到更高层次的抽象——从 prompt 到 context，再到系统架构。
...
![三代范式演进：Prompt Engineering 问"说什么"，Context Engineering 问"提供什么信息"，Harness Engineering 问"构建什么系统"](figures/fpa-01-three-generations.svg)
...
![仅修改 Harness 的基准测试对照：相同模型与 prompt 下成功率从 42% 跃升至 78%](figures/fpa-03-harness-leap.svg)
...
> **本章核心**：五种工作流模式是"从简单到复杂"的渐进式工具箱——最成功的实现用简单、可组合的模式构建，而非复杂框架。
...
![五种工作流模式：提示链、路由、并行化、编排器-工作者、评估器-优化器](figures/fpa-02-workflow-patterns.svg)
...
> **本章核心**：Agent 的能力主要来自系统架构而非模型微调——技术栈优先级是 prompt+RAG → 工具生态 → 知识库 → 微调。
...
> **本章核心**：Multi-Agent 是最后的选择而非第一选择——15× token 开销与编排失败风险，意味着只有明确的架构瓶颈才值得升级。
...
> **本章核心**：可观测性已是标配而非可选项——"LLM 评测确定 Agent 是否能工作；Agent 可观测性确定 Agent 是否在工作"。
...
> **本章核心**：分阶段部署是跨越生产鸿沟的最佳实践——影子模式、金丝雀、渐进式推出，外加硬断路器与默认拒绝的操作允许列表。
...
> **本章核心**：三条反复被验证的原则——从简单开始、投资 ACI 如同投资 HCI、模型无法可靠地评估自己的工作。
...
> **本章核心**：大多数生产场景只需要 L3-L4 工程成熟度；不要跳级——每一级的工程基础设施是下一级的前提。
