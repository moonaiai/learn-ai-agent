...
`scripts/extract-courses.ts` 扫描仓库根的 `courses/<课程id>/`，产出
`courses.json`，路由挂在 `/courses/` 下。课程与文档是两套平行管线：文档是
扁平主题分组，课程是 **模块 → 课时** 的有序树，课时页的侧边导航和上下篇都
限定在课程内。

新增一门课程只需要：

1. 把上游内容镜像到 `courses/<课程id>/`（目录与文件名原样保留，序号即顺序）
2. 写一个 `course.json`（标题、来源仓库、讲师、排序）

约定：一级子目录 `N. 模块名` 识别为模块；模块内 `N.M 课时名.md` 识别为课时
（按数字排序，`5.10` 不会排到 `5.2` 前面）；`N.M xxx/` 形式的数字子目录识别为
实验课，不出页面、在课程目录里链接到 GitHub。图片整树拷到
`public/course-assets/<id>/`，`.ipynb`/`.py` 等链接重写成 GitHub blob。
